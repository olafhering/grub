...
static struct argp_option options[] = {
  {"iterations",  'i', N_("NUM"), 0, N_("Number of iterations to perform"), 0},
  {"memory",      'm', N_("NUM"), 0, N_("Amount of memory (in kibibytes) to use"), 0},
  {"parallelism", 'p', N_("NUM"), 0, N_("Degree of parallelism (i.e. number of threads)"), 0},
...
argp_parser (int key, char *arg, struct argp_state *state)
{
  ...
  switch (key)
    {
    case 'i':
      arguments->iterations = strtoul (arg, NULL, 0);
      break;

    case 'm':
      arguments->memory = strtoul (arg, NULL, 0);
      break;

    case 'p':
      arguments->parallelism = strtoul (arg, NULL, 0);
      break;
...
static struct argp argp = {
  ...
  N_("Generate ARGON2 password hash."),
  NULL, NULL, NULL
};
...
main (int argc, char *argv[])
{
  ...
  param[0] = arguments.buflen;
  param[1] = arguments.iterations;
  param[2] = arguments.memory;
  param[3] = arguments.parallelism;
  gcry_err = grub_crypto_argon2 (GRUB_GCRY_KDF_ARGON2ID, param, 4,
				 pass1, grub_strlen (pass1),
				 salt, arguments.saltlen,
				 NULL, 0, NULL, 0,
				 arguments.buflen, buf);
  memset (pass1, 0, sizeof (pass1));
  ...
  /*
   * Output format: grub.argon2.<iterations>.<memory>.<parallelism>.<salt>.<hash>
   *
   * Reserve 20 digits for each argument: iterations, memory, and parallelism
   */
  result = xmalloc (sizeof ("grub.argon2.X.X.X.S.S") + (20 * 3) +
		    arguments.buflen * 2 + arguments.saltlen * 2);
  ptr = result;
  memcpy (ptr, "grub.argon2.", sizeof ("grub.argon2.") - 1);
  ptr += sizeof ("grub.argon2.") - 1;

  grub_snprintf (ptr, 20, "%ld", arguments.iterations);
  ptr += strlen (ptr);
  *ptr++ = '.';
  grub_snprintf (ptr, 20, "%ld", arguments.memory);
  ptr += strlen (ptr);
  *ptr++ = '.';
  grub_snprintf (ptr, 20, "%ld", arguments.parallelism);
  ptr += strlen (ptr);
  *ptr++ = '.';
  ...
  *ptr = '\0';
  ...
  printf (_("ARGON2 hash of your password is %s\n"), result);
  memset (buf, 0, arguments.buflen);
  free (buf);
  memset (salt, 0, arguments.saltlen);
  free (salt);
