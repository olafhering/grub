...
grub_efinet_create_dhcp_ack_from_device_path (grub_efi_device_path_t *dp, int *use_ipv6)
{
  ...
  if (GRUB_EFI_DEVICE_PATH_SUBTYPE (ldp) == GRUB_EFI_IPV4_DEVICE_PATH_SUBTYPE)
      ...
      if (err)
	{
      ...
      if (uri_len >= sizeof (bp->boot_file))
	{
	  grub_free (ddp);
	  grub_netbuff_free (nb);
	  return NULL;
      ...
      if (dns)
	{
	  ...
	  if (size_dns > GRUB_UCHAR_MAX)
	    {
	      num_dns = GRUB_UCHAR_MAX / sizeof (*dns);
	      size_dns = sizeof (*dns) * num_dns;
	    }
      ...
      if (err)
	{
      ...
      *use_ipv6 = 0;
      ...
      while (GRUB_EFI_DEVICE_PATH_TYPE (ldp) == GRUB_EFI_MESSAGING_DEVICE_PATH_TYPE
	     && GRUB_EFI_DEVICE_PATH_SUBTYPE (ldp) == GRUB_EFI_VLAN_DEVICE_PATH_SUBTYPE)
	{
	  ldp->type = GRUB_EFI_END_DEVICE_PATH_TYPE;
	  ldp->subtype = GRUB_EFI_END_ENTIRE_DEVICE_PATH_SUBTYPE;
	  ldp->length = sizeof (*ldp);
      ...
      if (GRUB_EFI_DEVICE_PATH_TYPE (ldp) == GRUB_EFI_MESSAGING_DEVICE_PATH_TYPE
	  && GRUB_EFI_DEVICE_PATH_SUBTYPE (ldp) == GRUB_EFI_MAC_ADDRESS_DEVICE_PATH_SUBTYPE)
	{
